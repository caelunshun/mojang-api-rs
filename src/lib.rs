...
pub fn server_auth(username: &str, server_hash: &str) -> Result<ServerAuthResponse, AuthError> {
    let url = format!("https://sessionserver.mojang.com/session/minecraft/hashJoined?username={}&serverId={}", username, server_hash);
    let client = Client::new();

    let mut res = client.get(&url)
        .send().map_err(|_| AuthError::RequestFailed)?;
...
mod tests {
    ...
    fn test_hexdigest() {
        ...
        assert_eq!(mc_hexdigest(hasher), "4ed1f46bbe04bc756bcb17c0c7ce3e4632f06a48");

        let mut hasher = Sha1::new();
        hasher.update(b"jeb_");
        assert_eq!(mc_hexdigest(hasher), "-7c9d5b0044c130109a5d7b5fb5c317c02b4e28c1");

        let mut hasher = Sha1::new();
        hasher.update(b"simon");
        assert_eq!(mc_hexdigest(hasher), "88e16a1019277b15d58faf0541e11910eb756f6");
    }
}
